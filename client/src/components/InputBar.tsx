...
const InputBar: React.FC<Props> = ({
    ...
    // const IndexModal = dynamic(() => impor('@/components/index/index-model').then((mod) => mod.IndexModal), {
    //     loading: () => <></>,
    // });

    // const { isSearch, isShadcnUI, showMindMap, setIsSearch, setIsShadcnUI, setShowMindMap } = useUIStore();
    const isSearch = true // todo remove
    return (
                ...
                <div className="flex relative">
                    <div className="absolute left-0 bottom-0 mb-1 ml-2 mt-6 flex items-center space-x-4">
                            ...
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    ...
                                    >
                                        ...
                                        <span className="font-serif text-sm  font-semibold dark:text-white">{('index-button')}</span>
                                    </button>
                                </TooltipTrigger>
                                <TooltipContent>{('index-tip')}</TooltipContent>
                            </Tooltip>
                        )} */}
                        ...
                        <Tooltip>
                                    ...
                                    >
                                        {isUploading ? (
                                            ...
                                            <div className="flex items-center dark:text-white">
                                                ...
                                                {searchType === SearchType.SEARCH && (
                                                    <span className="font-semibold font-serif text-sm ">{('attach-button')}</span>
                                                )}
                                            </div>
                                        )}
                                    </button>
                            ...
                            <TooltipContent>{searchType === SearchType.SEARCH ? ('attach-tip') : ('image-tip')}</TooltipContent>
                        </Tooltip>
                    </div>
                    <div className="absolute right-0 bottom-0 mb-1 mr-2 mt-6 flex items-center space-x-4">
                        <Tooltip>
                            ...
                            <TooltipContent>{('search-tip')}</TooltipContent>
                        </Tooltip>
                    </div>
                </div>
            </div>
